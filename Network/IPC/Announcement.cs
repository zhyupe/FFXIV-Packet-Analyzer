...
namespace PacketAnalyzer.Network.IPC
{
    ...
    class Announcement : IPCBase
    {
        public byte Unknown;
        public string Content;

        public Announcement(byte[] message, int offset)
        {
            Unknown = message[offset];
            Content = Encoding.UTF8.GetString(message, offset + 1, message.Length - (offset + 1)).TrimEnd((char)0);
        }

        public Announcement WriteParams(Dictionary<string, string> parsedValues)
        {
